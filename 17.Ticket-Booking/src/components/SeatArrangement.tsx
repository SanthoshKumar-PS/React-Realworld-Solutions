...
const seatingConfig : seatingConfigType[] = [
  ...
  {type: "VIP", rows: 2, seatsPerRow: 12, price: 500 }
];
const seatsLabel = [
    {label:'Selected',color:'bg-green-500'},
    {label:'Regular',color:'bg-blue-200'},
    {label:'Premium',color:'bg-pink-200'},
    {label:'VIP',color:'bg-yellow-200'},
        ...
        <div className='flex flex-wrap justify-center items-center gap-6 mt-4'>
            {seatsLabel.map(seatLabel=>(
                <div key={seatLabel.label} className='flex flex-col items-center justify-center space-y-1'>
                    <div className={`w-8 h-8 rounded-t-md ${seatLabel.color}`}/>
                    <p className='font-medium text-lg'>{seatLabel.label}</p>
                </div>
            ))}
        </div>
